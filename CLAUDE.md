...
An image retrieval tool that fetches images from the Internet Archive based on natural language prompts. Three-stage pipeline: (1) semantic bridge converts prompt to keywords, (2) `IAClient` searches the Internet Archive, (3) `Lomax` orchestrator downloads matching image files with metadata.
...
- `src/lomax/cli.py` — CLI entry point (registered as `lomax` console script). Minimal stub.
